  ...
  try {
    const report = await prisma.$transaction(async tx => {
      ...
      const [rfqs, inquiries, inquiryItems, supplierQuotes, quotations, orders, rfqLines, quotationLines, orderLines] = await Promise.all([
        ...
        tx.order.findMany({ select: { id: true, lineItemsMode: true, quotationId: true, partNumber: true, quantity: true, totalAmount: true, totalAmountDecimal: true, outboundQuantity: true, outboundStatus: true, inventoryDetailId: true, serialNumber: true, batchNumber: true } }),
