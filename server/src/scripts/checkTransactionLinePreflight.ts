...
try {
  const report = await prisma.$transaction(async tx => {
    ...
    const rfqs = await tx.rFQ.findMany({ select: { id: true, lineItemsMode: true, partNumber: true, alternatePartNumbers: true, quantity: true, requiredDate: true, certificateRequired: true, targetPriceCurrency: true } });
    const inquiries = await tx.inquiry.findMany({ select: { id: true, supplierId: true, rfqId: true } });
    ...
    const quotations = await tx.quotation.findMany({ select: { id: true, lineItemsMode: true, rfqId: true, partNumber: true, quantity: true, unitPrice: true, unitPriceDecimal: true, totalPrice: true, totalPriceDecimal: true, costPrice: true, costPriceDecimal: true, currency: true } });
    const orders = await tx.order.findMany({ select: { id: true, lineItemsMode: true, quotationId: true, partNumber: true, quantity: true, totalAmount: true, totalAmountDecimal: true, outboundQuantity: true } });
