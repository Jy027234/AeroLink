...
type Demand = Item & { requiredDate: Date; certificateRequired: boolean; targetPriceCurrency: string; alternatePartNumbers?: string | null };
...
export type LinePreflightInput = {
  rfqs: Demand[];
  inquiries: { id: string; supplierId: string }[];
  inquiryItems: InquiryItem[];
  supplierQuotes: (Priced & { rfqId: string | null; inquiryId: string | null; supplierId: string })[];
  quotations: (Priced & { rfqId: string; currency: string; costPrice: number; costPriceDecimal: MoneyInput | null })[];
  orders: (Item & { quotationId: string; totalAmount: number; totalAmountDecimal: MoneyInput | null; outboundQuantity: number })[];
};
...
export function assessTransactionLineMigration(input: LinePreflightInput) {
  ...
  const rfqs = new Map(input.rfqs.map(row => [row.id, row]));
  const inquiries = new Map(input.inquiries.map(row => [row.id, row]));
  const quotations = new Map(input.quotations.map(row => [row.id, row]));
  const itemGroups = new Map<string, InquiryItem[]>();
  for (const item of input.inquiryItems) itemGroups.set(item.inquiryId, [...(itemGroups.get(item.inquiryId) ?? []), item]);
  ...
  const inquiryCandidates = input.inquiries.map(inquiry => {
    const items = itemGroups.get(inquiry.id) ?? [];
    const candidates = items.length === 1 ? input.rfqs.filter(rfq => sameDemand(items[0], rfq)).map(rfq => rfq.id) : [];
  ...
  for (const entity of ['supplierQuotes', 'quotations'] as const) {
    for (const quote of input[entity]) {
  ...
  return {
    ...
    counts: Object.fromEntries(Object.entries(input).map(([entity, rows]) => [entity, rows.length])),
