...
以上改动已通过后端 81 项测试、前端 17 项测试及构建/lint、两套 Prisma Schema 校验和两套 Compose 配置解析；它们还没有替代正式的生产迁移基线、TLS 终止、依赖升级和真实隔离环境 E2E，因此不会改变本文的当前 No-Go 结论。
...
| 后端 Vitest | 81/81 通过 | 14 个测试文件（含模板净化、RFQ、订单状态、API Key 权限、刷新 Cookie 与会话版本回归） |
...
- 报价提交和审批没有完整校验当前状态，可能从不合理状态直接跳转：`server/src/routes/quotations.ts:375-438`。
