...
以上改动已通过后端 85 项测试、前端 17 项测试及构建/lint、两套 Prisma Schema 校验和两套 Compose 配置解析；真实隔离环境 E2E 已完成数据库准备和构建，但上一轮导航用例因未展开侧栏分组失败，修复后已重新触发 CI，结果待回收。因此它们还没有替代正式的生产迁移基线、TLS 终止和依赖升级，也不会改变本文的当前 No-Go 结论。
...
| 后端 Vitest | 85/85 通过 | 15 个测试文件（含模板净化、RFQ、报价/订单状态、API Key 权限、刷新 Cookie 与会话版本回归） |
...
- 基线版本的报价提交和审批没有完整校验当前状态，可能从不合理状态直接跳转；本轮新增 `server/src/lib/quotationStateMachine.ts` 并接入 `server/src/routes/quotations.ts:375-468`，非法迁移统一返回 409，同态重试不重复产生审批事件。报价发送、接受、撤回的状态历史、迁移原因码和并发版本控制仍需补齐。
