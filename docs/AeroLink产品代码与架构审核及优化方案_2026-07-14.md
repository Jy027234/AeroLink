...
- 生产入口默认使用受基线门禁保护的 `prisma migrate deploy`；SQLite 兼容 profile 才允许显式 `SCHEMA_MIGRATION_MODE=push`。
- 生产入口默认关闭演示 seed；seed 必须显式提供 `DEMO_SEED_PASSWORD`，并通过一次性管理员激活流程满足活动管理员门禁。
...
- 生产启动入口增加订单唯一性 preflight：空库自动跳过；发现重复 `quotationId` 时在 schema 变更前阻断启动并输出待清理记录，紧急情况下可显式使用 `SKIP_SCHEMA_PREFLIGHT=true` 绕过。
...
- AI Agent、AI 模型和工作流定义的写操作统一增加经理级及以上 RBAC；设置页隐藏普通业务账号的 AI 配置入口；RBAC 拒绝统一返回 `AUTH_FORBIDDEN`，并兼容历史 `administrator` 角色别名。
...
- 浏览器 access token 改为仅保存在内存；刷新只依赖 HttpOnly refresh Cookie，页面刷新通过 `/auth/refresh` + `/auth/me` 恢复会话，旧的本地 token 只在退出/失效恢复时清理。
- API 客户端增加保留分页 envelope 的通道，审计日志、工作流实例、Webhook、RFQ、报价、订单、客户、供应商和库存列表开始保留 `pagination`；这些主列表默认页大小提高到 100，后续仍需把页面筛选完全迁移到服务端。
- RFQ 管理页继续将状态、紧急度、搜索和页码下推到服务端查询，页面按服务端 `pagination` 翻页，创建/编辑/状态变更成功后重新拉取当前页；其他大列表仍按后续批次迁移。
- 报价管理页继续将状态页签、搜索和页码下推到服务端查询，增加报价状态汇总和已接受金额汇总，审批/发送/撤回/建单后刷新当前页。
- 订单管理页继续将进行中/已完成状态分组、搜索和页码下推到服务端查询，并增加订单总量、进行中、已完成和总金额汇总。
- 库存管理页继续将件号/描述/序列号搜索、状态/证书/类型/分类/库位筛选和页码下推到服务端查询，增加分类、库位和库存价值汇总；同时补齐列表响应缺失的分类与跟踪字段，跨页勾选不再丢失已选库存。
- 客户管理页继续将状态、名称/联系人/邮箱搜索和页码下推到服务端查询，并增加客户状态与年度收入汇总。
- 供应商管理页继续将等级、名称/联系人/邮箱搜索、跟进条件和页码下推到服务端查询，并增加供应商等级与绩效汇总。
- Webhook 管理页的 Phase 2 DLQ 列表接入服务端 `offset/limit` 分页，显示总数与当前范围；重试或放弃导致末页收缩时自动回退到有效页，避免隔离消息超过 20 条后不可浏览。
- Webhook 审计日志时间范围筛选改为服务端 `createdAt` 条件，投递/审计列表在筛选或数据收缩后自动回退到有效页，避免只过滤当前页造成总数失真或空白页。
- Webhook 投递记录和 DLQ 管理新增端点筛选；DLQ 新增失败原因筛选，并统一前端契约使用服务端实际返回的 `quarantineAt/attemptCount/lastError` 字段，修复页面显示 `undefined` 的问题。
- 库存双模型新增只读数量对账纯函数和数据库检查脚本，按件号汇总 `Inventory.quantity` 与 `InventoryItem.details.quantity`，发现差异时输出明细并以非零状态退出；未改动任何线上库存写入路径。
- 库存管理 API 新增 manager/gm/admin 权限保护的只读对账接口，库存页增加“运行对账”入口与差异摘要；只展示诊断结果，不自动修复或改写库存。
- 导航回归 E2E 按实际可折叠侧栏分组展开后再定位子项，避免把 UI 的初始折叠状态误判为页面不可用。
- Agent 演示链路保留真实 RFQ 创建和人工跟进审计；demo 分支将人工触达候选稳定显示为 `Skyline Aero Trading`，但继续复用真实供应商 ID，避免跟进日志脱离主数据。

以上改动已通过后端 109 项测试、前端 21 项测试及构建/lint、两套 Prisma Schema 校验和生产 Compose 配置解析；Webhook DLQ 分页、筛选与契约字段，库存双模型对账纯函数和只读 API，以及报价确认并发幂等回归也已通过验证。此前 CI run #13 已在隔离 PostgreSQL 中通过导航、报价/合同及全量 E2E。本地 access token 改造还需下一次隔离环境 E2E 验证；这些改动仍没有替代正式的生产迁移基线、证书签发、历史密钥清理和正式发布演练，也不会改变本文的当前 No-Go 结论。

### 1.5 批次一新增落地（本地工作区，未提交/未推送）

- `deploy/server-entrypoint.sh` 增加 schema 模式门禁：PostgreSQL 默认 `migrate deploy`，只有完成备份/演练并显式设置 `MIGRATION_BASELINE_CONFIRMED=true` 才会执行；生产环境拒绝未授权的 `db push`。
- 增加 `server/src/scripts/bootstrapAdmin.ts` 与 `db:bootstrap-admin`：创建无默认密码、默认未激活的管理员并输出一次性激活链接；`db:check-active-admin` 阻止无活动特权账号的服务启动。
- `server/src/seed.ts` 移除硬编码演示密码，要求隔离环境显式提供 `DEMO_SEED_PASSWORD`；生产 seed 仍需额外设置 `ALLOW_PRODUCTION_DEMO_SEED=true`。
- CI 增加 Gitleaks Secret Scan，并为隔离 E2E 显式注入临时 seed/E2E 口令；E2E 脚本不再内置 `password123`。
- `.env.production.example`、README 与 Compose profile 已同步上述门禁和初始化流程。真实密钥轮换、历史清理、PostgreSQL 基线确认、TLS 和生产发布仍需在外部受控环境完成，本地不会代替执行。

### 1.6 批次二新增落地（本地工作区，未提交/未推送）

- `deploy/nginx.tls.conf` 与 `docker-compose.prod.tls.yml` 提供可选 HTTPS profile：HTTP 统一 301 到 HTTPS，仅开放 TLS 1.2/1.3，并保留安全响应头；默认 8080 兼容 profile 不被改写。
- `deploy/server-entrypoint.sh` 增加 `REQUIRE_SECURE_ORIGIN` 门禁：生产 Secure refresh cookie 不再允许误配到非 HTTPS 公网来源；本地开发/CI（非 production）不受影响。
- `server/src/index.ts` 收紧生产 CORS/Socket.IO 来源：production 只接受显式 `CLIENT_URL`，localhost 开发来源仅在非 production 环境保留。
- `deploy/verify-production.sh` 提供只读发布验收，检查 `/api/health` 以及 HTTPS 下的 HSTS、CSP、X-Content-Type-Options、X-Frame-Options、Referrer-Policy 和 Permissions-Policy。
- `deploy/Dockerfile.web` 增加 `NGINX_CONFIG` 构建参数，使 HTTP/TLS 配置共享同一前端镜像构建链路。
- `.dockerignore` 排除本地数据库、上传目录、依赖、构建产物、截图和 secrets；本地 TLS 镜像构建上下文已从约 476 MB 降至约 21 KB，避免把开发机数据带入构建上下文。
- 官方 npm registry 的前后端生产依赖审计均为 0 个漏洞；后端锁文件已升级 `multer`、`morgan`、`mailparser`、`express/qs`、`socket.io/ws` 等安全修复版本，`nodemailer` 升至 9.0.3，并通过 `utf7 -> semver@5.7.2` 的兼容覆盖清除 IMAP 链路遗留高危依赖。
- 前后端开发依赖的间接告警也已收敛：前端 Vite 升至 7.3.6，并将 `@babel/core`、`js-yaml`、`undici`、`ws`、`vite`/`esbuild`、`form-data` 与 IMAP 链路 semver 锁定到修复版本；当前前端与后端执行完整 `npm audit` 均为 0 个漏洞。
- CI 的前端/后端作业均增加 `npm audit --registry=https://registry.npmjs.org` 全量门禁，避免镜像站审计接口不可用或依赖漏洞被静默带入构建。
- Deploy 工作流已收敛为 `Build and Verify`：校验生产/TLS Compose 并构建 HTTP、TLS、backend 镜像但不 push；真实镜像仓库、部署凭据、回滚目标和证书由后续受控环境配置。

### 1.7 云端测试环境 P0 验收（2026-07-15）

在用户明确“云端仅为测试环境”后，已将本地已落地的 P0 改动同步到 `101.43.50.30:/home/ubuntu/aerolink-prod`，完成隔离验证、发布、自动回滚和人工回滚演练。域名、TLS 和公网安全组按要求未执行，测试环境继续使用 HTTP 端口 8080。

- 加密密钥已轮换，旧密钥备份已删除；历史扫描未发现有效的 64 位十六进制密钥；当前密钥文件权限为 `600`。本次测试库无待重加密凭据，轮换脚本仍保留为可审计的 dry-run/apply 流程。
- PostgreSQL 已建立基线并执行 `prisma migrate deploy`，5 条迁移均已完成；生产运行参数为 `SCHEMA_MIGRATION_MODE=migrate`、`ALLOW_DB_PUSH=false`、`SEED_DEMO_DATA=false`。
- 库存数据迁移后数量对账通过：`Inventory=7`、`InventoryItem=6`、`InventoryDetail=7`，两套数量合计均为 `43`；订单 `quotationId` 唯一索引存在，活动特权账号检查通过。
- Clean Runner E2E 连续 3 次通过，每次 21/21，用于覆盖认证、导航、RFQ、报价、合同及 UI 审计链路。
- 云端发布脚本已支持发布记录、PostgreSQL 备份、SBOM、健康检查和自动回滚；已完成正常发布、坏镜像自动回滚和人工回滚演练，备份与 SBOM 文件均以 `600` 权限保存。
- Deploy 工作流新增前后端 SPDX SBOM 生成与上传门禁；真实镜像仓库推送仍需后续配置仓库凭据，不在本次测试环境闭环内执行。

以上结果证明“不含域名/TLS”的 P0 测试环境闭环已完成；不代表公网生产上线批准，也不替代外部渗透测试、真实证书签发或合规认证。

### 1.8 下一阶段核心交易一致性（本地工作区，未提交/未推送）

- 报价确认和直接建单都以唯一的 `quotationId` 作为资源级幂等键；并发请求撞到数据库唯一约束时回读已创建订单，不再返回误导性的重复冲突。
- 重试已复用已有合同文档，并跳过重复的 `quotation.accepted` / `order.created` Webhook；新增 Prisma 唯一约束错误识别 helper 和并发冲突回归测试。
- 该方案不新增 Redis 或幂等表，适用于当前“一个报价只生成一个订单”的业务约束；跨资源批量写入、库存扣减和外部 Idempotency-Key 仍需后续 Outbox/幂等存储方案。
...
| 前端 Vitest | 21/21 通过 | 6 个测试文件，含内存 access token、刷新和 401 重试回归 |
| 后端 `npm run build` | 通过 | TypeScript 编译通过 |
| 后端 Vitest | 109/109 通过 | 26 个测试文件（新增 Prisma 唯一约束错误识别与报价确认并发幂等回归） |
...
| 后端生产依赖审计 | 0 个漏洞 | `npm audit --omit=dev --registry=https://registry.npmjs.org` |
| 后端全量依赖审计 | 0 个漏洞 | `npm audit --registry=https://registry.npmjs.org`（含开发依赖） |
| 缺少刷新密钥启动实验 | 按预期失败 | 报错：`JWT_SECRET and JWT_REFRESH_SECRET must be set` |
| 批次一 seed 安全门禁 | 通过 | 未提供 `DEMO_SEED_PASSWORD` 时在任何数据删除前失败；不再使用硬编码密码 |
| 生产入口 Shell 语法 | 通过 | `bash -n deploy/server-entrypoint.sh` |
| 两套生产 Compose 解析 | 通过 | PostgreSQL 默认 migrate；SQLite profile 明确 push；环境变量门禁可展开 |
| TLS Compose 覆盖层解析 | 通过 | `docker-compose.prod.tls.yml` 仅覆盖 web 端口为 80/443，并挂载只读证书目录 |
| TLS 前端镜像构建 | 通过 | `docker build --build-arg NGINX_CONFIG=deploy/nginx.tls.conf`，不 push |
| 后端镜像构建 | 通过 | `docker build --file deploy/Dockerfile.server`，包含 `npm ci`、Prisma Client 生成与 TypeScript 构建，不 push |
| Docker 构建上下文 | 通过 | `.dockerignore` 排除本地数据库、依赖、上传文件和 secrets |
| 发布验收脚本语法 | 通过 | `bash -n deploy/verify-production.sh`；未连接生产地址执行写操作 |
| 生产 HTTPS 来源门禁 | 通过 | 非 HTTPS `CLIENT_URL` 在 production entrypoint 中拒绝启动；可由受控环境显式关闭 |
| Git 敏感文件检查 | 部分完成 | 工作区已停止跟踪 `secrets/encryption_key.txt`（本地文件保留）；历史轮换/清理和 Gitleaks 全历史复测仍需受控流程 |

### 2.3 未执行项与边界

- 未连接或检查线上服务器、线上数据库、日志、备份、域名证书和云安全组。
- 未验证真实 SMTP/IMAP、OpenAI、物流承运商、ERP 或外部数据源。
- 本地未执行会写入当前业务数据库的全量 E2E；CI 已在隔离 PostgreSQL 中通过导航、报价/合同及全量 E2E。真实生产数据、SMTP/IMAP 和外部系统仍未验证。
- 订单唯一性 preflight 本次仅使用本地 SQLite 数据库验证通过；未使用真实生产 PostgreSQL 凭据执行检查，正式切库前仍需在备份/演练环境运行。
- 未进行专业渗透测试、软件成分分析平台复核或航空适航合规认证；本地未安装 Gitleaks，因此只接入 CI 门禁，未在工作区执行二进制扫描。
...
| P0-06 | Deploy 工作流当前只完成构建，没有镜像发布、部署或回滚。见 `.github/workflows/deploy.yml:7-40` | 实际发布链路仍需受控环境配置 | 构建不可变镜像、生成 SBOM、推送仓库、部署、冒烟、失败回滚；本地仅保留构建与验收门禁 | 发布记录可追溯到 commit 和镜像 digest；冒烟失败自动回滚 |
| P0-07 | 基线生产示例直接使用 HTTP；本地已提供可选 `docker-compose.prod.tls.yml`，但真实证书和域名尚未配置 | 登录凭据、业务数据和令牌在未切 TLS 时可能被链路窃听或篡改 | 使用受信任证书叠加 TLS profile；HTTP 自动 301；外部扫描只允许 TLS 1.2+ | 外部扫描只允许 TLS 1.2+；HTTP 自动 301；安全头测试通过 |
| P0-08 | 基线审计曾发现后端 High/Moderate 漏洞；当前官方 npm registry 复测为 0 个生产漏洞 | 依赖风险已暂时清零，但需持续监控新公告 | 保留锁文件与 CI 审计；新增漏洞必须记录升级或风险例外 | 生产依赖 High/Critical 为 0，或每项都有批准的风险接受与补偿控制 |
...
本轮已补充 AI Agent、AI 模型和工作流定义的管理权限门槛，设置页同步隐藏普通业务账号的 AI 配置入口，并将通用 RBAC 拒绝统一为 `AUTH_FORBIDDEN`；后续仍需继续把权限矩阵收敛为资源级 capabilities，并处理并列角色与资源所有权。
...
历史版本的 RFQ、报价、订单、库存、客户和供应商列表默认分页 20 条，并把分页放在响应顶层；旧的通用 `request()` 只返回 `data`，会丢弃同级 `pagination`。`DataInitializer` 再把有限结果写入全局 store，各页面最后做一次客户端分页。

本轮已为审计日志、工作流实例、Webhook、RFQ、报价、订单、客户、供应商和库存列表增加保留 envelope 的请求通道，并将这些主列表默认页大小提高到 100；RFQ、报价、订单、库存、客户和供应商管理页已完成服务端筛选/分页及汇总，审计日志和工作流实例页面已有服务端分页，Webhook 管理页已补齐 DLQ 列表的 `offset/limit` 分页、投递/审计时间范围服务端过滤、投递/审计/DLQ 末页回退，以及投递端点和 DLQ 端点/失败原因筛选。部分 Webhook 回放列表仍需后续统一 URL 驱动的筛选状态，超过 100 条仍需继续验证。

证据：

- 服务端分页与搜索：`server/src/routes/rfqs.ts:78-119`；其他主资源仍采用相同分页模式但页面尚未全部接入。
- 报价服务端分页与搜索：`server/src/routes/quotations.ts:88-158`。
- 订单服务端分页与搜索：`server/src/routes/orders.ts:15-92`。
- 库存服务端分页与搜索：`server/src/routes/inventory.ts:15-148`。
- 客户服务端分页与搜索：`server/src/routes/customers.ts:78-137`。
- 供应商服务端分页与搜索：`server/src/routes/suppliers.ts:122-190`。
- Webhook DLQ 服务端分页：`server/src/routes/webhooks-phase2.ts:32-64`。
- Webhook 投递/审计服务端筛选：`server/src/routes/inboundWebhooks.ts:205-271`。
- Webhook DLQ 端点/失败原因筛选：`server/src/routes/webhooks-phase2.ts:32-64`。
- 通用解包与 envelope 通道：`src/api/client.ts:547-588`。
- 全局初始化：`src/components/DataInitializer.tsx:20-68`。
- RFQ 页面服务端分页：`src/sections/RFQManagement/index.tsx:879-942`。
- 报价页面服务端分页：`src/sections/Quotations/index.tsx:1382-1448`。
- 订单页面服务端分页：`src/sections/Orders/index.tsx:1118-1165`。
- 库存页面服务端分页：`src/sections/Inventory/index.tsx:1428-1538`。
- 客户页面服务端分页：`src/sections/Customers/index.tsx:1001-1048`。
- 供应商页面服务端分页：`src/sections/Suppliers/index.tsx:731-830`。
- 审计日志、工作流实例和 Webhook DLQ 页面分页：`src/sections/AuditLogs/index.tsx:531-559`、`src/sections/Workflows/index.tsx:662-671,1031-1053`、`src/sections/Settings/WebhookManagementPanel.tsx:131-225,871-944`。
- Webhook 审计日期参数：`src/api/client.ts:2233-2272`。
- Webhook 投递/DLQ 筛选和 DLQ 字段契约：`src/sections/Settings/WebhookManagementPanel.tsx:125-233,739-1029`、`src/api/client.ts:688-708,2001-2040`。

未迁移的页面仍可能把首屏数据误当作完整数据集；上述主列表和运维列表已经不再依赖客户端切片访问下一页，但 Webhook 投递导出全量化和回放列表仍需后续统一筛选状态与分页边界。
...
- 当前报价确认与直接建单已将 `quotationId` 作为资源级幂等键：唯一约束冲突会回读已创建订单，重复请求复用合同且不重复发送 `quotation.accepted` / `order.created` Webhook；仍需为跨资源批量写入和库存扣减补充持久化 `Idempotency-Key` 与 Outbox。
...
- 数量对账实现：`server/src/lib/inventoryReconciliation.ts`、`server/src/scripts/checkInventoryReconciliation.ts`；命令：`npm run db:check:inventory-reconciliation`。
- 对账管理 API 与页面：`server/src/routes/inventory.ts:151-178`、`src/api/client.ts:1424-1430`、`src/sections/Inventory/index.tsx:1440-1475,1830-1885`。
...
在迁移完成前，禁止两个模型同时接受写入；选定一个主模型，并用对账脚本每日验证差异为 0。本轮已先落地只读对账能力：`server/src/lib/inventoryReconciliation.ts` 负责按件号聚合和差异计算，`server/src/scripts/checkInventoryReconciliation.ts` 通过 `npm run db:check:inventory-reconciliation` 执行检查；同时新增 `GET /api/inventory/reconciliation`（manager/gm/admin）和库存页“运行对账”入口。发现差异时输出件号、两侧数量和 delta 并返回非零状态，表不存在时安全跳过；接口与页面只展示结果，不自动改写库存。
...
本轮已完成主要收敛：前端 401 使用单飞刷新；刷新接口优先读取 `HttpOnly + SameSite=Lax` Cookie，兼容旧客户端在请求体传入 refresh token；登录、激活、重置密码和刷新都会轮换 Cookie，登出会清除 Cookie。JWT 增加用户会话版本号，密码修改、密码重置和账号启停会递增版本并拒绝旧令牌。浏览器 access token 已改为内存态，不再写入 `localStorage`；页面刷新会通过 refresh Cookie 恢复会话，并新增登录、刷新、退出及 401 重试四类前端回归测试。设备级会话记录和会话管理界面仍待后续完成。
...
当前 46 个路由文件对应 22 个后端测试文件，28 个前端业务模块对应 6 个前端测试文件。测试最集中在认证、报价/合同和 Agent runtime，库存、权限矩阵、证书、物流、VMI、拍卖、寄售、定价和 Webhook 的核心异常分支覆盖不足。
